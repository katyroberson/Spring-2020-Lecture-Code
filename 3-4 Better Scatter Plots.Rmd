...
colorVals <- rep("grey", nrow(mid))
colorVals[mid$average_pledge <= lowPledge] <- "red"

plot(mid$usd_goal_real, mid$usd_pledged_real, col=colorVals)
lab <- rep("", nrow(mid))
lab[mid$average_pledge <= lowPledge] <- mid$name[mid$average_pledge <= lowPledge]
text(mid$usd_goal_real, mid$usd_pledged_real, label=lab, cex=.5)
```
...
```{r}
plot(mid$usd_goal_real, mid$usd_pledged_real, col=colors[categoryIndex], pch=1)
```
...
```{r}
plot(catGoalAvg, catPledgedAvg)
text(catGoalAvg, catPledgedAvg-300, labels=names(catGoalAvg), cex=.5)
```

# Correlation
The closer it is to +/- 1, the stronger the relationship
```{r}
m <- df[, c("usd_goal_real", "usd_pledged_real")]
#cor(m)
cor(mid[, c("usd_pledged_real", "usd_goal_real")])
```
...
```{r}
plot(mid$usd_goal_real, mid$usd_pledged_real, col="grey")
